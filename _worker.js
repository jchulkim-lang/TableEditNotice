...
function renderBoard(){
  ...
  const adminBars = ME.is_admin ? \`
    <div class="bar">
      ...
      <span class="pill" id="svnMsg">관리자만 수정 가능 · 알림에도 함께 표시</span>
    </div>
    <div class="bar">
      <label>테이블 추가</label>
      <input id="newTable" placeholder="파일명 (예: boss_table.xlsx)" style="width:230px">
      <input id="newMemo" placeholder="설명(선택)" style="flex:1;min-width:120px">
      <button class="btn btn-primary" onclick="addTable()">추가</button>
    </div>\` : "";
  $("#app").innerHTML=\`
    <h1>SVN 테이블 편집 보드</h1>
...
async function addTable(){
  const name=($("#newTable").value||"").trim(); const memo=($("#newMemo").value||"").trim();
  if(!name){ toast("테이블 파일명을 입력하세요."); return; }
  const r=await fetch("/api/tables/add",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({table_name:name,memo})});
  if(!r.ok){const d=await r.json();toast(esc(d.error||"추가 실패"));return;}
  $("#newTable").value=""; $("#newMemo").value=""; refresh();
}
async function removeTable(name){
  if(!confirm(\`'\${name}' 을(를) 목록에서 삭제할까요?\`)) return;
  const r=await fetch("/api/tables/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({table_name:name})});
  if(!r.ok){const d=await r.json();toast(esc(d.error||"삭제 실패"));return;}
  refresh();
}
